    ...
    <Card className="group hover:shadow-xl hover:shadow-accent/20 transition-all duration-300 hover:-translate-y-2 relative overflow-hidden border-accent/30 hover:border-accent/60 bg-card/80 backdrop-blur-sm glow-border">
        ...
        <div className="relative h-48 overflow-hidden">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <Button
              size="lg"
              onClick={handlePlay}
              className="bg-accent/90 hover:bg-accent backdrop-blur-sm shadow-lg transform scale-90 group-hover:scale-100 transition-transform duration-200 glow-border"
            >
          ...
          {episode.duration && (
            <div className="absolute bottom-3 right-3">
              <Badge variant="secondary" className="bg-background/90 backdrop-blur-sm text-xs border-accent/30">
                {episode.duration}
              </Badge>
            </div>
          )}
      ...
      {!episode.imageUrl && (
        <div className="h-48 bg-gradient-to-br from-muted via-muted/80 to-muted/60 relative overflow-hidden">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,_transparent_0%,_transparent_40%,_var(--accent)_100%)] opacity-10" />
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="p-4 rounded-full bg-accent/20 border border-accent/40 glow-border">
              <Play size={32} className="text-accent/80 glow-text" weight="duotone" />
            </div>
          </div>
          {episode.duration && (
            <div className="absolute bottom-3 right-3">
      ...
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <Button
            ...
            variant="ghost"
            onClick={handlePlay}
            className="shrink-0 opacity-60 group-hover:opacity-100 transition-opacity duration-200 hover:text-accent"
          >
